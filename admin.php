...
if(!empty($_FILES['minu_fail']['name'])){
    ...
    if (in_array($_FILES['minu_fail']['type'], $lubatudlaiendus)) {
        if (is_writable($kataloog)) {
            ...
            $previous_file = $kataloog.'/a.jpg';
            if (file_exists($previous_file)) {
                unlink($previous_file);
            }
            
            if(move_uploaded_file($ajutine_fail, $kataloog.'/a.jpg')){
...
<h1>Pricing muutmine</h1>
<form action="" method="post" enctype="multipart/form-data">
<input type="file" name="minu_fail1" accept=".jpg, .jpeg" required>
<input type="submit" value="Lae üles!">

</form>
<?php
if(!empty($_FILES['minu_fail1']['name'])){
    $sinu_faili_nimi = $_FILES['minu_fail1']['name'];
    $ajutine_fail= $_FILES['minu_fail1']['tmp_name'];
    
    $faili_suurus = $_FILES['minu_fail1']['size'];
    $lubatudlaiendus = array("image/jpeg", "image/jpg");
    ...
    if (in_array($_FILES['minu_fail1']['type'], $lubatudlaiendus)) {
        if (is_writable($kataloog)) {
            
            $previous_file = $kataloog.'/b.jpg';
            if (file_exists($previous_file)) {
                unlink($previous_file);
            }
            
            if(move_uploaded_file($ajutine_fail, $kataloog.'/b.jpg')){
                echo 'Faili üleslaadimine oli edukas';
            } else {
                echo 'Faili üleslaadimine ebaõnnestus';
            }
